...
ROOT_DIR = os.path.abspath("/content/drive/current-lane/mrcnn")
...
def apply_mask(image, mask, color, alpha=0.5):  #alpha   toumingdu
    """Apply the given mask to the image.
    """
    for c in range(3):
        image[:, :, c] = np.where(mask == 1,  # mask = 0 background ;mask = 1 road
                                  image[:, :, c] *
...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    for i in range(N):
        #i want to output the fixed colour,RGB~(0,1)
        # color = [[1,1,1]]  #Green [[0,0.8,0]]
        color = [[0,0.8,0]]
        ...
        # Mask
        mask = masks[:, :, i]
        masked_image = apply_mask(masked_image, mask, color[class_id-1])
